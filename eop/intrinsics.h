...
template <typename T>
requires(Regular(T)) void construct(T &p)
{
...
requires(Regular(T) && Constructible(T, U)) void construct(T &p, const U &initializer)
{
    // Precondition: $p$ refers to raw memory, not an object
...
template <typename T>
requires(Regular(T)) void destroy(T &p)
{
...
template <typename T, typename U>
requires(Regular(T)) void destroy(T &p, U &finalizer)
{
    // Precondition: $p$ is in a partially-formed state
    // Postcondition: $p$ refers to raw memory, not an object
