...
struct measure_time
{
    ...
    N n;
    clock_t t;
    measure_time(const pointer(char) legend) : inverse_accuracy(100),
...
struct measure_reverse_bidirectional
{
    ...
    pointer(T) f;
    pointer(T) l;
    measure_reverse_bidirectional() : legend("10 repetitions of reverse_bidirectional(f, f+10000000)"),
                                      N(10000000),
...
struct measure_sort_linked
{
    ...
    measure_sort_linked() : legend("sort_linked(reverse(iota(100000)))"),
                            N(100000),
                            REPETITIONS(1)
    {
...
void measure_sort_n_adaptive_compares()
{
    for (int n = 64; n <= 32768; n *= 4)
    {
        ...
        print(" compares for ");
        print(int(size(a)));
        print(" elements; ratio = ");
        print(double(c) / double(size(a)));
        print_eol();
    }
}
...
struct measure_sort_n_adaptive
{
    ...
    pointer(T) l;
    array<T> b;
    measure_sort_n_adaptive() : legend("sort_n_adaptive(reverse(iota(100000)))"),
                                N(100000),
                                REPETITIONS(1),
                                a(N, N, T(17)),
                                f(begin(a)), l(end(a)),
...
template <typename M>
requires(Measure(M)) void report(M m)
{
    print(m.legend);
    print(": ");
    print(time_per_iter(m));
    print(" ");
    print(m.units);
    print(" ");
    print(m.n);
