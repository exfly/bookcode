...
requires(Transformation(F) && T == Domain(F) &&
         UnaryPredicate(P) && Domain(F) == Domain(P)) void output_orbit_structure(Domain(F) x, F f, P p)
{
    triple<DistanceType(F), DistanceType(F), Domain(F)> t =
        orbit_structure(x, f, p);
    if (!p(t.m2))
    {
        print("terminating with h-1 = ");
        print(t.m0);
        print(" and terminal point ");
        print(t.m2);
    }
    else if (t.m2 == x)
    {
        print("circular with collision point ");
        print(collision_point(x, f, p));
        print(" and c-1 = ");
        print(t.m1);
    }
    else
    {
        print("rho-shaped with collision point ");
        print(collision_point(x, f, p));
        print(" and h = ");
        print(t.m0);
        print(" and c-1 = ");
        print(t.m1);
        print(" and connection point ");
        print(t.m2);
    }
    print_eol();
}
...
template <typename I>
requires(Integer(I)) struct additive_congruential_transformation
{
    I modulus;
    I index;
    additive_congruential_transformation(I modulus, I index) : modulus(modulus), index(index) {}
    I operator()(I x) { return remainder(x + index, modulus); }
};

template <typename I>
requires(Integer(I)) struct input_type<additive_congruential_transformation<I>, 0>
{
    typedef I type;
};
...
template <typename I>
requires(Readable(I) && IndexedIterator(I) && ValueType(I) == DistanceType(I)) struct table_transformation
{
...
template <typename I>
requires(Readable(I) && IndexedIterator(I) && ValueType(I) == DistanceType(I)) struct table_transformation_definition_space_predicate
{
...
template <typename I>
requires(Readable(I) && IndexedIterator(I) && ValueType(I) == DistanceType(I)) struct input_type<table_transformation<I>, 0>
{
    typedef ValueType(I) type;
};

template <typename I>
requires(Readable(I) && IndexedIterator(I) && ValueType(I) == DistanceType(I)) struct distance_type<table_transformation<I>>
{
...
template <typename T>
requires(Regular(T)) void push(array<T> &x, const T &y)
{
...
struct LCG // linear congruential generator
{
    ...
    LCG(T m, T a, T b, T x0, const pointer(char) name) : m(m), a(a), b(b), x0(x0), name(name) {}
...
template <typename T>
requires(DiscreteEuclideanSemiring(T)) struct multiplies_modulo
{
    T m;
...
template <typename T>
requires(DiscreteEuclideanSemiring(T)) struct input_type<multiplies_modulo<T>, 0>
{
    typedef T type;
};
...
void run_idempotent_power()
{
    ...
    while (true)
    {
        ...
        print("idempotent_power()(");
        print(n);
        print(", multiplication modulo ");
        print(m);
        print(") == ");
        print(p);
        print_eol();
...
void run_fibonacci()
{
    ...
    while (true)
    {
        ...
        print(n);
        print(") == ");
        print(fibonacci<long long>(n));
        print_eol();
    }
}

template <typename T>
requires(AdditiveMonoid(T)) struct annotated_plus
{
...
template <typename T>
requires(AdditiveMonoid(T)) struct input_type<annotated_plus<T>, 0>
{
    typedef T type;
};

template <typename T>
requires(AdditiveMonoid(T)) struct codomain_type<annotated_plus<T>>
{
    typedef T type;
};

template <typename T>
requires(AdditiveMonoid(T)) struct annotated_negate
{
...
template <typename T>
requires(EuclideanSemiring(T)) struct modulus
{
    T operator()(const T &x, const T &y) const { return x % y; }
};

template <typename T>
requires(EuclideanSemiring(T)) struct input_type<modulus<T>, 0>
{
    typedef T type;
};

template <typename T>
requires(EuclideanSemiring(T)) struct quo_rem
{
...
template <typename T>
requires(EuclideanSemiring(T)) struct input_type<quo_rem<T>, 0>
{
    typedef T type;
};
...
void run_quotient_remainder()
{
    ...
    while (true)
    {
        ...
        print(slow_remainder(a, b));
        print_eol();
        print("remainder_nonnegative_with_largest_doubling = ");
        print(remainder_nonnegative_with_largest_doubling(a, b));
        print_eol();
        print("                                  remainder = ");
        print(remainder(a, b, modulus<int>()));
        print_eol();
        pair<int, int> p;
...
void run_gcd()
{
    ...
    while (true)
    {
        ...
        print(fast_subtractive_gcd(a, b));
        print_eol();
        print("gcd<int, int> = ");
        print(gcd<int, int>(a, b));
        print_eol();
        print("binary_gcd_nonnegative(abs(), abs()) = ");
        print(stein_gcd_nonnegative(abs(a), abs(b)));
        print_eol();
    }
}
...
template <typename T>
requires(Regular(T)) struct verify_conservation
{
    const T *t;
    T t_0;
...
void run_slist_tests()
{
    ...
    print(l2);
    print_eol();
    print("partition not odd: ");
    print(l1);
    print_eol();
    erase_all(l1);
    merge_copy_n(a1, 3, a2, 3, insert_iterator<after<slist<int>>>(after<slist<int>>(l1, end(l1))), less<int>());
    print("merge copy: ");
    print(l1);
    print_eol();
...
void run_list_tests()
{
    ...
    print_eol();
    erase_all(l1);
    merge_copy_n(a1, 3, a2, 3, insert_iterator<after<list<int>>>(after<list<int>>(l1, end(l1))), less<int>());
    print("merge copy: ");
    print(l1);
    print_eol();
...
template <typename C>
requires(EmptyLinkedBifurcateCoordinate(C)) struct serializer
{
...
template <typename C>
requires(EmptyLinkedBifurcateCoordinate(C)) void stree_serialize(C c)
{
    traverse_rotating(c, serializer<C>());
}

template <typename C>
requires(EmptyLinkedBifurcateCoordinate(C)) void tree_serialize(C c)
{
    traverse_rotating(c, serializer<C>());
}
...
void run_stree_tests()
{
    ...
    print(t);
    print_eol();
    traverse_rotating(begin(t3_45), print_coordinate<I>);
    print_eol();
    traverse_rotating(begin(t3_45), print_coordinate<I>);
    print_eol();
    traverse_rotating(begin(t3_45), print_coordinate<I>);
    ...
    traverse_rotating(begin(t3_45), print_coordinate<I>);
    print_eol();
    traverse_rotating(begin(t2_345_678), print_coordinate<I>);
    print_eol();
    traverse_rotating(begin(t), print_coordinate<I>);
    print_eol();
    print("stree_serialize of t3_45: ");
    stree_serialize(begin(t3_45));
    print_eol();
    T u(t);
    print("Preorder  traversal of t: ");
    traverse(t, print_visit<I>(true, false, false));
    print_eol();
    print("Inorder   traversal of t: ");
    traverse(t, print_visit<I>(false, true, false));
    print_eol();
    print("Postorder traversal of t: ");
    traverse(t, print_visit<I>(false, false, true));
    print_eol();
    Assert(t == u);
    Assert(!(t < u) && !(u < t));
    Assert(T() < T(0));
    ...
    Assert(!(T(0, T(0), T(0)) < T(0, T(0), T())));
    {
        ...
        push(a, T(0));
        {
            ...
            print("sl before sorting: ");
            print(sl);
            print_eol();
            sort(sl, less<T>());
            print("sl after  sorting: ");
            print(sl);
            print_eol();
...
void run_tree_tests()
{
    ...
    traverse_rotating(begin(t), print_coordinate<I>);
    print_eol();
    print("tree_serialize of t3_45: ");
    tree_serialize(begin(t3_45));
    print_eol();
    T u(t);
    print("Preorder  traversal of t: ");
    traverse(t, print_visit<I>(true, false, false));
...
requires(Relation(R) && Mutable(I) && Integer(ValueType(I))) struct instrumented_less
{
    R r;
    I p;
...
template <typename R, typename I>
requires(Relation(R) && Mutable(I) && Integer(ValueType(I))) struct input_type<instrumented_less<R, I>, 0>
{
    typedef Domain(R) type;
};

template <typename T>
requires(Regular(T)) struct tracer
{
    T t;
    tracer(const T &t) : t(t)
    {
        print("Constructing tracer(");
        print(t);
        print(")\n");
    }
